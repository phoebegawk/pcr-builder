...
def extract_month_year_from_excel(file_bytes: bytes) -> str:
    """
    Reads the uploaded PCR Numbers workbook and extracts the date directly
    underneath the 'ENDED' column header, then formats it as 'Month Year'.
    """
    ...
    for sheet in workbook.worksheets:
        ...
        raise ValueError(
            "I found the 'ENDED' header, but the cell below it wasn't a valid date."
        )
...
def build_pcr_pptx(client_name: str, month_year: str) -> BytesIO:
    if not PPTX_TEMPLATE_PATH.exists():
        raise FileNotFoundError(
            f"Template not found at: {PPTX_TEMPLATE_PATH}"
        )
            ...
            buildBtn.addEventListener("click", async () => {
                ...
                try {
                    ...
                    if (!response.ok) {
                        ...
                        throw new Error(errorText || "Something went wrong building the PCR.");
                    }
...
async def build_pcr(
    ...
    try:
        ...
        pptx_stream = build_pcr_pptx(client_name=client_name, month_year=month_year)
